	...
	methods: {
		reload () {
			...
			if (this.value.length) {
				for (let i in this.value) {
					console.log(this.levelcodeToArray(this.value[i]))
		...
		add () {
			if (this.value.indexOf(this.tpData[this.tpData.length -1]) === -1) {
				...
				this.search(this.selectOption, this.levelcodeToArray(this.tpData[this.tpData.length - 1]))
			} else {
				this.$message({
					type: 'warning',
					message: '不能重复添加！'
		...
		search (data, value) {
			const self = this
			for (let i in data) {
				if (self.strLength(data[i].value) !== self.searchLevel + 1) {
					self.searchLevel = self.strLength(data[i].value) - 1
				}
