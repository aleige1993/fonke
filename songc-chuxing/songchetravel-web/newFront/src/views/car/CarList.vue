<template>
    ...
    <m-table :titleKey="titleKey" :tableData="tableData" :opreatWidth="110" :tableList="tableList" :showOther="false" localName="carList_table" :otherList="detialDic" :keyword="keyword" :opreat="opreat" :total="total" @pageChange="pageChange" :add="roles.add" @add="add"></m-table>
		<!--dialog add-->
		<el-dialog :title="optTitle" :visible.sync="addFormVisible" :before-close="goCancelAdd" :close-on-click-modal="false">
...
<script>
	...
	export default {
      ...
      methods: {
        rolesMethod () {
          const self = this
          self.$store.dispatch('checkApi', ['/car/vehicle/add', '/car/vehicle/update', '/car/vehicle/delete']).then(data => {
            self.roles = {
              add: data[0]
            }
