<template>
	<div>
		...
		<div class="block m-sch">
			<el-form :inline="true" :model="formSch" class="demo-form-inline fl">
				<el-form-item >
					<div class="el-form-item__content">
						<el-select @change="handleSchBrandChange" v-model="formSch.brand" clearable placeholder="车辆品牌">
							<el-option v-for="itm in schOptions.brand" :key="itm.label" :label="itm.label" :value="itm">
							</el-option>
						</el-select>
					</div>
				</el-form-item>
				<el-form-item v-if="schOptions.series.length">
					<div class="el-form-item__content">
						<el-select clearable placeholder="车系" v-model="formSch.series" @change="handleSchSeriesChange">
							<el-option v-for="itm in schOptions.series" :key="itm.label" :label="itm.label" :value="itm">
							</el-option>
						</el-select>
					</div>
				</el-form-item>
				<el-form-item label="名称">
					<el-input v-model="formSch.name" placeholder="名称"></el-input>
				</el-form-item>
				<el-form-item>
					<el-button type="primary" @click="handelSch">查询</el-button>
				</el-form-item>
			</el-form>
			<div class="fr" v-if="roles.add">
				<el-button type="primary" @click="goAdd"><i class="iconfont xy-btn-icon">&#xe624;</i>添加</el-button>
			</div>
		</div>
			...
			<el-table :data="tableData" border style="width: 100%">
				<el-table-column type="expand">
					<template slot-scope="props">
						<el-form label-position="right" inline  class="m-table-expand">
							<div class="electric" v-if="props.row.electric == false">
								<el-form-item label="排量:">
									<span>{{ props.row.displacement_value }} {{props.row.displacement_type | displacement_type}}</span>
								</el-form-item>
								<el-form-item label="变速箱分类:">
									<span>{{ props.row.gearbox_type | gearbox_type }}</span>
								</el-form-item>
								<el-form-item label="燃油标号:">
									<span>{{ props.row.oil_type | oil_type }}</span>
								</el-form-item>
								<!-- <el-form-item label="发动机类型:"><span>{{ props.row.displacement_type | displacement_type }}</span></el-form-item> -->
								<el-form-item label="油箱容量（L）:">
									<span>{{ props.row.tankage}}</span>
								</el-form-item>
							</div>
							<el-form-item label="最大续航里程(公里):">
								<span>{{ props.row.max_mileage}}</span>
							</el-form-item>
							<el-form-item label="是否定点还车:">
								<span>{{ props.row.fix_location ? "是" : "否"}}</span>
							</el-form-item>
						</el-form>
					</template>
				</el-table-column>
				<el-table-column label="车型图片">
					<template slot-scope="scope">
						<a class="modelImg" href="javascript:;" :style="{'background-image': 'url(' + baseImgUrl+scope.row.vm_pic_url + ')'}" preview-nav-enable="false" v-preview="(baseImgUrl+scope.row.vm_pic_url)">
						</a>
					</template>
				</el-table-column>
				<el-table-column prop="name" label="名称">
				</el-table-column>
				<el-table-column label="品牌>车系">
					<template slot-scope="scope">
						{{ scope.row.brandname }} > {{ scope.row.serialname }}
					</template>
				</el-table-column>
				<el-table-column label="机动车类型">
					<template slot-scope="scope">
						{{ scope.row.electric | electric }}
					</template>
				</el-table-column>
				<el-table-column prop="seat_number" label="座位数">
				</el-table-column>
				<el-table-column prop="model_year" label="生产年份">
				</el-table-column>
				<el-table-column label="细分类型">
					<template slot-scope="scope">
						{{scope.row.classfy | classfy}}
					</template>
				</el-table-column>
				<el-table-column prop="time_price" label="按时间 价格(元/分钟)">
				</el-table-column>
	           <el-table-column prop="deposit" label="押金(元)">
				</el-table-column>
				<el-table-column label="操作" width="175" fixed="right">
					<template slot-scope="scope">
						<div class="opreat-table">
							<el-button size="small" type="text" v-if="roles.edit" @click="goEdit(scope.$index, scope.row)">编辑</el-button>
							<el-button size="small" type="text" v-if="roles.setFee" @click="goPrice(scope.$index, scope.row)">定价</el-button>
							<el-button size="small" type="text" v-if="roles.setDeposit" @click="goDeposit(scope.$index, scope.row)">押金</el-button>
							<el-button size="small" type="text" v-if="roles.delete" class="delete" @click="handleDelete(scope.$index, scope.row)">删除</el-button>	
						</div>
					</template>
				</el-table-column>
			</el-table>
				...
				<div class="pagination">
					<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :page-sizes="[15,20, 50, 100]" :current-Page="pageIndex" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
					</el-pagination>
				</div>

				<!--dialog add-->
				<el-dialog :title="optTitle" :visible.sync="addFormVisible" :before-close="goCancelAdd" :size="'large'" :close-on-click-modal="false">
					<h2 class="dialog-title" slot="title"><i class="iconfont c-yellow">&#xe624;</i>{{optTitle}}</h2>
	...
	export default {
		data() {
			...
			methods: {
				rolesMethod () {
					const self = this
					self.$store.dispatch('checkApi', ['/car/vehicleModel/add', '/car/vehicleModel/update', '/car/vehicleModel/delete', '/car/vehicleModel/setFee', '/car/vehicleModel/setDeposit']).then(data => {
						self.roles = {
							add: data[0],
							edit: data[1],
							delete: data[2],
