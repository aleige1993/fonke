<template>
	<div class="activity">
		...
		<m-table :tableData="tableData" :opreatWidth="110" :tableList="tableList" localName="activity_table" :otherList="detialDic" :keyword="keyword" :opreat="opreat" :total="total" @pageChange="pageChange"></m-table>
		<!--dialog add-->
		<el-dialog title="添加" :visible.sync="addFormVisible">
			...
			<el-form class="dialog-form clearfix" :model="formAdd" ref="formAdd" :rules="ruleAdd">
				...
				<el-form-item label="图片：" :label-width="formLabelWidth" prop="des_img_url">
					...
					<el-upload :on-success="handleAddPic" class="m-avatar-uploader" :thumbnail-mode="true" :action="APIADDRESS + '/sys/sys/uploadFile'" :show-file-list="false" :before-upload="beforeAvatarUpload">
		...
		<el-dialog :visible.sync="editFormVisible" :before-close="goCancelEdit">
			...
			<el-form class="dialog-form clearfix" :model="formEdit" ref="formEdit" :rules="ruleEdit">
				...
				<el-form-item label="图片：" :label-width="formLabelWidth" prop="des_img_url">
					<!--:on-success="handleBrandPic" :before-upload="beforeAvatarUpload"-->
					<el-upload :on-success="handleEditPic" class="m-avatar-uploader" :thumbnail-mode="true" :action="APIADDRESS + '/sys/sys/uploadFile'" :show-file-list="false" :before-upload="beforeAvatarUpload">
	...
	export default {
		...
		data() {
			...
			return {
				...
				endTimeOption: {
          disabledDate(time) {
            return time.getTime() < new Date(self.formAdd.end_time) - 8.64e7;
          }
        },
