<template>
	<div class="modle-list">
		...
		<m-table :tableData="tableData" labelWidth="130px" :opreatWidth="110" :tableList="tableList" localName="carModel_table" :otherList="detialDic" :keyword="keyword" :opreat="opreat" :total="total" @pageChange="pageChange"></m-table>
		...
		<el-dialog :title="optTitle" :visible.sync="addFormVisible" :before-close="goCancelAdd" :size="'large'" :close-on-click-modal="false">
			...
			<el-form :model="formAdd" ref="formAdd" :rules="ruleAdd">
				<el-row>
					<el-col :span="8">
						<el-form-item label="关联的品牌车系" prop="sirui_vmid" :label-width="formLabelWidth">
							<el-popover ref="popoverAdd" placement="right" trigger="click">
								<el-form-item label="品牌" :label-width="formLabelWidth">
									...
									<el-button @click="addBrand" type="primary">新增品牌</el-button>
								</el-form-item>
								<el-form-item label="车系" v-if="seriesShow" :label-width="formLabelWidth">
                  ...
                  <el-button @click="addSeries" type="primary">新增车系</el-button>
								</el-form-item>
				...
				<el-row>
					...
					<el-col :span="8">
						<el-form-item label="定点还车" :label-width="formLabelWidth" prop="fix_location">
							<el-select v-model="formAdd.fix_location" clearable placeholder="是否定点还车">
								<el-option label="定点还车" value="1"></el-option>
								<el-option label="非定点还车" value="0"></el-option>
								<el-option label="随停还车" value="2"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="8">
			...
			<div slot="footer" class="dialog-footer">
				...
				<el-button type="primary" :loading="btnLoad" @click="handleAdd('formAdd')">确 定</el-button>
			</div>
		</el-dialog>
		<el-dialog :visible.sync="showBrand">
			<h2 class="dialog-title" slot="title"><i class="iconfont c-yellow">&#xe624;</i>新增品牌</h2>
			<el-form :inline="true" :model="brandAdd" ref="brandAdd" :rules="brandRule">
				<el-form-item label="品牌中文名：" prop="cn_name">
					<el-input type="text" v-model="brandAdd.cn_name" />
				</el-form-item>
				<el-form-item label="品牌英文名：" prop="en_name">
					<el-input type="text" v-model="brandAdd.en_name" />
				</el-form-item>
				<el-form-item>
					<el-button @click="addBrandFun" type="primary">确定</el-button>
				</el-form-item>
			</el-form>
		</el-dialog>
		<el-dialog :visible.sync="showSeries">
			<h2 class="dialog-title" slot="title"><i class="iconfont c-yellow">&#xe624;</i>新增车系</h2>
			<el-form :inline="true" :model="seriesAdd" ref="seriesAdd" :rules="seriesRule">
				<el-form-item label="车系中文名：" prop="cn_name">
					<el-input type="text" v-model="seriesAdd.cn_name" />
				</el-form-item>
				<el-form-item label="车系英文名：" prop="en_name">
					<el-input type="text" v-model="seriesAdd.en_name" />
				</el-form-item>
				<el-form-item>
					<el-button @click="addSeriesFun" type="primary">确定</el-button>
				</el-form-item>
			</el-form>
		</el-dialog>
	</div>
...
<script>
	...
	import { setDeposit, addBrand, addSeries } from 'api/car'
	export default {
		data() {
			const self = this
			return {
        ...
        detialDic: [
          ...
          this.getData()
				...
				addBrandFun () {
					this.$refs['brandAdd'].validate((valid) => {
						if (valid) {
							addBrand(this.brandAdd).then(res => {
        ...
        addSeries () {
				  this.showSeries = true
					this.seriesAdd.brandId = this.formAdd.brand.id
        },
        addSeriesFun () {
          this.$refs['seriesAdd'].validate((valid) => {
            if (valid) {
              addSeries(this.seriesAdd).then(res => {
                if (res.data.result.resultCode == 0) {
                  this.$message.success('新增成功')
                  this.showSeries = false
                  this.getOptions(this.carType.series, this.seriesAdd.brandId);
                } else {
                  this.$message.error(res.data.result.resultMessage)
                }
              }).catch(err => {
		      ...
		      for (let i in this.filterParams) {
				...
				handleDelete(idx, itm) {
					...
					handleSeriesChange(itm) {
						...
						// //formAdd
						this.formAdd.vehicle_model = itm;
